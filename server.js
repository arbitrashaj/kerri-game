...
function buildHTML() {
  return [
    ...
    '/* ── FLIP CARDS ── */',
    '.flip-card{display:inline-flex;width:52px;height:74px;perspective:600px;cursor:pointer;flex-shrink:0}',
    '.flip-card-inner{position:relative;width:100%;height:100%;transition:transform .45s cubic-bezier(.4,0,.2,1);transform-style:preserve-3d}',
    '.flip-card.flipped .flip-card-inner{transform:rotateY(180deg)}',
    '.flip-card-front,.flip-card-back-face{position:absolute;inset:0;border-radius:8px;backface-visibility:hidden;-webkit-backface-visibility:hidden;display:flex;flex-direction:column;align-items:center;justify-content:center;box-shadow:var(--shadow-sm)}',
    '.flip-card-front{background:linear-gradient(135deg,#2d1f4e,#1e1535);border:.5px solid rgba(155,114,232,.4);font-size:20px;color:#9b72e8;user-select:none}',
    '.flip-card-back-face{background:linear-gradient(145deg,#242436,#1a1a2e);border:.5px solid rgba(255,255,255,.12);transform:rotateY(180deg)}',
    '.flip-card-back-face .rank{font-size:17px;font-weight:700;line-height:1}',
    '.flip-card-back-face .suit{font-size:13px;line-height:1;margin-top:3px}',
    '.flip-card-back-face.red-face .rank,.flip-card-back-face.red-face .suit{color:var(--red)}',
    '.flip-card-back-face.black-face .rank,.flip-card-back-face.black-face .suit{color:#f0e6c8}',
    '.flip-card-back-face.kerri-face{border-color:var(--kerri-br);background:var(--kerri-bg)}',
    '.flip-card-back-face.kerri-face .rank,.flip-card-back-face.kerri-face .suit{color:var(--kerri)}',
    '.flip-card:hover .flip-card-inner{transform:rotateY(180deg)}',
    '.flip-card.flipped:hover .flip-card-inner{transform:rotateY(180deg)}',
    '  </style>',
    '</head>',
    '<body>',
    '',
    ...
    '    var fc=card.isKerri?"kerri-face":RED_SUITS[card.suit]?"red-face":"black-face";',
    '    return "<div class=\\\"flip-card\\\" onclick=\\\"flipCard(this)\\\">" +"<div class=\\\"flip-card-inner\\\">" +"<div class=\\\"flip-card-front\\\">\\u2660</div>" +"<div class=\\\"flip-card-back-face "+fc+"\\\"><div class=\\\"rank\\\">"+card.rank+"</div><div class=\\\"suit\\\">"+card.suit+"</div></div>" +"</div></div>";',
    '  }).join("");',
    '}',
