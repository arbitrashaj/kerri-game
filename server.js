...
function buildHTML() {
  return [
    ...
    '#phase-game .seat-cards{display:flex;align-items:flex-end;justify-content:center;height:60px;min-width:60px;flex-shrink:0;padding:8px 4px 0}',
    '#phase-game .seat-card-back{position:absolute;width:30px;height:42px;border-radius:4px;border:.5px solid rgba(155,114,232,.5);background:linear-gradient(135deg,#2d1f4e,#1e1535);display:flex;align-items:center;justify-content:center;font-size:11px;color:#9b72e8;transition:transform .18s,box-shadow .18s;box-shadow:1px 1px 4px rgba(0,0,0,.5)}',
...
'function cardTap(idx){',
    ...
    '  var pos=angles.map(function(a){',
    '    var r=a*Math.PI/180;',
    '    var lp=Math.round(50+40*Math.sin(r));',
    '    var tp=Math.round(50-32*Math.cos(r));',
    '    return {t:tp+"%",l:lp+"%",x:"translate(-50%,-50%)"}',
    '  });',
    '  state.opponents.forEach(function(opp,seatIdx){',
    '    var p=pos[seatIdx%pos.length];',
    '    var iD=state.isMyTurn&&opp.id===state.drawFromId&&opp.cardCount>0;',
    '    var av="seat-avatar"+(iD?" active-seat":opp.isOut?" out-seat":opp.disconnected?" disc-seat":"");',
    '    var av="seat-avatar"+(iD?" active-seat":opp.isOut?" out-seat":opp.disconnected?" disc-seat":"");',
    '    var oppN=opp.cardCount;var oppMaxA=Math.min(25,oppN*3);',
    '    for(var k=0;k<opp.cardCount;k++){',
    '      var cc="seat-card-back"+(opp.disconnected?" disc-card":iD?" drawable-seat":"");',
    '      var offsetX=k*Math.min(10, 60/Math.max(opp.cardCount,1));',
    '      var oppA=oppN>1?(k/(oppN-1)-0.5)*oppMaxA:0;',
    '      var ck=iD?" data-id=\'"+ escAttr(opp.id) +"\' data-idx="+k+" onclick=\'cardFlyDraw(this,"+k+")\'":"";',
    '      cards+="<div class=\'"+ cc +"\'"+ ck +" style=\'transform:rotate("+oppA+"deg) translateY("+oppLift+"px);transform-origin:bottom center;display:inline-block\'>&#9824;</div>";',
    '    }',
    ...
    '    var cardLabel=opp.isOut?"<span style=\'color:var(--green);font-size:10px\'>&#10003; Doli</span>":opp.cardCount+" letra";',
    '    seat.innerHTML="<div class=\'"+ av +"\'>"+escHtml(opp.name.charAt(0).toUpperCase())+"</div>"+"<div class=\'seat-name\'>"+escHtml(opp.name)+"</div>"+"<div class=\'seat-cards\'>"+cards+"</div>"+"<div style=\'font-size:10px;color:var(--hint);text-align:center\'>"+cardLabel+"</div>";',
    '    area.appendChild(seat);',
    '  });',
    '}',
    'function cardFlyDraw(el,idx){',
    '  var fId=el.dataset.id;',
    '  var rect=el.getBoundingClientRect();',
    '  var myHand=document.getElementById("my-hand");',
    '  var destRect=myHand?myHand.getBoundingClientRect():{left:window.innerWidth/2,top:window.innerHeight-100};',
    ...
    '    requestAnimationFrame(function(){',
    '      var dx=destRect.left+destRect.width/2-rect.left-rect.width/2;',
    '      var dy=destRect.top-rect.top;',
    '      fly.style.cssText+="transition:transform .45s cubic-bezier(.4,0,.2,1),opacity .45s;transform:translate("+dx+"px,"+dy+"px) scale(.8);opacity:0";',
    '    });',
    '  });',
    '  setTimeout(function(){fly.remove();},500);',
    '  sendMsg("draw_card",{fromPlayerId:fId,cardIndex:idx});SFX.cardDraw();',
    '}',
