...
function buildHTML() {
  return [
    ...
    '#phase-game .seat-card-back{width:24px;height:34px;border-radius:3px;border:.5px solid rgba(155,114,232,.4);background:linear-gradient(135deg,#2d1f4e,#1e1535);display:inline-flex;align-items:center;justify-content:center;font-size:10px;color:#9b72e8;transition:transform .1s}',
    '#phase-game .seat-card-back.drawable-seat{cursor:pointer;border-color:var(--gold);color:var(--gold);box-shadow:0 0 6px rgba(212,168,67,.4)}',
    '#phase-game .seat-card-back.drawable-seat:hover{transform:translateY(-4px) scale(1.15)}',
    ...
    '      var el="<div class=\'"+ cc +"\'"+(iD?" data-id=\'"+ escAttr(opp.id) +"\' data-nm=\'"+ escAttr(opp.name) +"\' data-ct="+ opp.cardCount +" onclick=\'openDrawModal(this.dataset.id,this.dataset.nm,+this.dataset.ct)\'":"")+">&#9824;</div>";',
    '      cards+=el;',
    '    }',
