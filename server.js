...
function buildHTML() {
  return [
    ...
    '#phase-game .seat-cards{display:flex;flex-wrap:wrap;gap:2px;justify-content:center;max-width:90px}',
    '#phase-game .seat-card-back{width:32px;height:46px;border-radius:4px;border:.5px solid rgba(155,114,232,.4);background:linear-gradient(135deg,#2d1f4e,#1e1535);display:inline-flex;align-items:center;justify-content:center;font-size:13px;color:#9b72e8;transition:transform .15s,box-shadow .15s;cursor:default;user-select:none}',
    '#phase-game .seat-card-back.drawable-seat{cursor:pointer;border-color:var(--gold);color:var(--gold);box-shadow:0 0 8px rgba(212,168,67,.5);animation:card-pick .9s ease-in-out infinite alternate}',
    '#phase-game .seat-card-back.drawable-seat:hover{transform:translateY(-8px) scale(1.2);box-shadow:0 8px 20px rgba(212,168,67,.6);border-color:#fff}',
    '#phase-game .seat-card-back.disc-card{opacity:.25}',
    ...
    '#phase-game .my-hand-cards{display:flex;flex-wrap:nowrap;gap:5px;overflow-x:auto;padding:2px 4px;scrollbar-width:none;justify-content:center;width:100%;max-width:100vw}',
    ...
    '.playing-card.dragging{opacity:.4;transform:scale(.95)}',
    '.playing-card.drag-over{border-color:var(--gold)!important;box-shadow:0 0 0 2px rgba(212,168,67,.5)}',
    ...
    'var prevHandIds=[];',
'var dragSrcIdx=null;',
'function reorderLocal(hand){',
    '  var ids=hand.map(function(c){return c.id+"_"+Math.random();});',
    '  return hand;',
    '}',
'function renderMyHand(hand){',
    '  var c=document.getElementById("my-hand"),cnt=document.getElementById("my-hand-count");',
    '  if(!hand||hand.length===0){c.innerHTML="✓ Ke dalët";if(cnt)cnt.textContent="";prevHandIds=[];return;}',
    ...
    '    var cls="playing-card draggable"+(card.isKerri?" kerri-card":RED_SUITS[card.suit]?" red-card":" black-card");',
    '    if(isNew&&i===hand.length-1)cls+=" just-drawn";',
    '    return "<div class=\\\""+cls+"\\\" draggable=\\\"true\\\" data-idx=\\\""+i+"\\\" ondragstart=\\\"cardDragStart(event,"+i+")\\\" ondragover=\\\"cardDragOver(event)\\\" ondrop=\\\"cardDrop(event,"+i+")\\\" ondragend=\\\"cardDragEnd(event)\\\">"+"<div class=\\\"rank\\\">"+card.rank+"</div>"+"<div class=\\\"suit\\\">"+card.suit+"</div></div>";',
    '  }).join("");',
    '  prevHandIds=newIds;',
    '}',
'function cardDragStart(e,idx){dragSrcIdx=idx;e.target.classList.add("dragging");e.dataTransfer.effectAllowed="move";}',
'function cardDragOver(e){e.preventDefault();e.target.closest(".playing-card").classList.add("drag-over");}',
'function cardDragEnd(e){document.querySelectorAll(".playing-card").forEach(function(c){c.classList.remove("dragging","drag-over");});}',
'function cardDrop(e,dropIdx){',
    '  e.preventDefault();',
    '  document.querySelectorAll(".playing-card").forEach(function(c){c.classList.remove("drag-over");});',
    '  if(dragSrcIdx===null||dragSrcIdx===dropIdx)return;',
    '  var hand=lastHandData;if(!hand)return;',
    '  var moved=hand.splice(dragSrcIdx,1)[0];',
    ...
    '  var pos=[{t:"6%",l:"50%",x:"translateX(-50%)"},{t:"28%",l:"83%",x:"translate(-50%,-50%)"},{t:"72%",l:"83%",x:"translate(-50%,-50%)"},{t:"72%",l:"17%",x:"translate(-50%,-50%)"},{t:"28%",l:"17%",x:"translate(-50%,-50%)"}];',
    '  state.opponents.forEach(function(opp,i){',
    ...
    '      var el="<div class=\'"+ cc +"\'"+(iD?" data-id=\'"+ escAttr(opp.id) +"\' data-idx="+ k +" onclick=\'confirmDraw(this.dataset.id,+this.dataset.idx)\'":"")+">&#9824;</div>";',
    '      cards+=el;',
    '    }',
    ...
    '    seat.innerHTML="<div class=\'"+ av +"\'>"+escHtml(opp.name.charAt(0).toUpperCase())+"</div><div class=\'seat-name\'>"+escHtml(opp.name)+"</div><div class=\'seat-cards\'>"+cards+"</div>";',
